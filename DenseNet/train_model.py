...
class Trainer(object):
    def __init__(self, args):
        ...
        # )
        model = DenseNet121(cfg.NUM_CLASSES)
        ...
        # Define lr scheduler
        self.scheduler = LR_Scheduler(args.lr_scheduler, args.lr, args.epochs, len(self.train_loader)) #这是一种根据验证过程中某个指标变化趋势进行调整的方法。
                                                                                                       #例如我需要在验证过程中计算模型的准确率，当准确率连续5次没有下降时，第6次训练就降低学习率。
    ...
    def training(self, epoch):
        ...
        for i, (image, target) in enumerate(tbar) :
            ...
            # Acc += (pred == target).sum().item() / total  #修改
...
def main():
    ...
    parser.add_argument('--workers', type=int, default=1, metavar='N', help='dataloader threads')  #小于batch_size
    ...
    parser.add_argument('--batch_size', type=int, default=2, metavar='N', help='input batch size for \
                                                                                training (default: auto)')  #大于2可能报错
    parser.add_argument('--test_batch_size', type=int, default=2, metavar='N', help='input batch size for \
    ...
    parser.add_argument('--no_val', action='store_true', default=False, help='skip validation during training')     #False
