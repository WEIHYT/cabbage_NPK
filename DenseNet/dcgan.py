...
parser.add_argument("--lr", type=float, default=0.0002, help="adam: learning rate")            #学习率 0.0002
...
parser.add_argument("--imgsize", type=int, default=64, help="size of each image dimension")  #输入数据的维度
parser.add_argument('--img_size', type=int, default=(64,64), help='train and val image resize')   #32*32
parser.add_argument("--channels", type=int, default=3, help="number of image channels")      #输入数据的通道数
parser.add_argument("--sample_interval", type=int, default=10, help="interval between image sampling")  #保存图像的迭代数
...
#             [transforms.Resize(opt.imgsize), transforms.ToTensor(), transforms.Normalize([0.5], [0.5])]
#         ),
#     ),
...
dataloader, _ ,_= make_data_loader(opt, **kwargs)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "/root/cabbage/DenseNet/dcgan2/%d.png" % batches_done, nrow=5, normalize=True)
            print("images/%d.png" % batches_done)
            img_list.append(vutils.make_grid(fake, padding=2, normalize=True))
        if epoch>opt.n_epochs-2 or epoch==1:
            save_image(gen_imgs.data[:1], "/root/cabbage/DenseNet/dcgan2/%d.png" % batches_done, nrow=1, normalize=True)
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('/root/cabbage/DenseNet/generate_loss2.jpg')
plt.show()
...
image=np.transpose(vutils.make_grid(real_batch[0].to(cuda)[:64], padding=5, normalize=True).cpu(),(1,2,0))
# plt.imshow()
...
# plt.imshow()
plt.savefig(image,'/root/cabbage/DenseNet/real_fake.jpg')
plt.show()
